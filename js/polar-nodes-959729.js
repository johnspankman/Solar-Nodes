...
/* globals window, document, navigator, lucidweb_ENV_TEST */

/* eslint-disable no-var */

/**
 * lucidweb: Core site library
 */

var lucidweb = {};
var modules = {};
var primary = [];
var secondary = window.lucidweb || [];
var $ = window.jQuery;
var $win = $(window);
var $doc = $(document);
var isFunction = $.isFunction;

var _ = lucidweb._ = __webpack_require__(132);

var tram = lucidweb.tram = __webpack_require__(68) && $.tram;
var domready = false;
var destroyed = false;
tram.config.hideBackface = false;
tram.config.keepInherited = true;
/**
 * lucidweb.define - Define a named module
 * @param  {string} name
...
lucidweb.define = function (name, factory, options) {
  if (modules[name]) {
...
/**
 * lucidweb.require - Require a named module
 * @param  {string} name
 * @return {object}
 */


lucidweb.require = function (name) {
  return modules[name];
};

function bindModule(module) {
  // If running in lucidweb app, subscribe to design/preview events
  if (lucidweb.env()) {
...
/**
 * lucidweb.push - Add a ready handler into secondary queue
 * @param {function} ready  Callback to invoke on domready
 */


lucidweb.push = function (ready) {
...
/**
 * lucidweb.env - Get the state of the lucidweb app
 * @param {string} mode [optional]
 * @return {boolean}
 */


lucidweb.env = function (mode) {
...
var touch = lucidweb.env.touch = 'ontouchstart' in window || window.DocumentTouch && document instanceof window.DocumentTouch;
var chrome = lucidweb.env.chrome = /chrome/.test(userAgent) && /Google/.test(navigator.vendor) && parseInt(userAgent.match(/chrome\/(\d+)\./)[1], 10);
var ios = lucidweb.env.ios = /(ipod|iphone|ipad)/.test(userAgent);
lucidweb.env.safari = /safari/.test(userAgent) && !chrome && !ios; // Maintain current touch target to prevent late clicks on touch devices
...
/**
 * lucidweb.validClick - validate click target against current touch target
 * @param  {HTMLElement} clickTarget  Element being clicked
 * @return {Boolean}  True if click target is valid (always true on non-touch)
 */

lucidweb.validClick = touch ? function (clickTarget) {
  return clickTarget === touchTarget || $.contains(clickTarget, touchTarget);
} : function () {
  return true;
};
/**
 * lucidweb.resize, lucidweb.scroll - throttled event proxies
 */

var resizeEvents = 'resize.lucidweb orientationchange.lucidweb load.lucidweb';
var scrollEvents = 'scroll.lucidweb ' + resizeEvents;
lucidweb.resize = eventProxy($win, resizeEvents);
lucidweb.scroll = eventProxy($win, scrollEvents);
lucidweb.redraw = eventProxy(); // Create a proxy instance for throttled events
...
function eventProxy(target, types) {
  ...
  return proxy;
} // lucidweb.location - Wrap window.location in api


lucidweb.location = function (url) {
  window.location = url;
};

if (lucidweb.env()) {
  // Ignore redirects inside a lucidweb design/edit environment
  lucidweb.location = function () {};
} // lucidweb.ready - Call primary and secondary handlers


lucidweb.ready = function () {
...
/**
 * lucidweb.load - Add a window load handler that will run even if load event has already happened
 * @param  {function} handler
 */
...
lucidweb.load = function (handler) {
...
} // lucidweb.destroy - Trigger a destroy event for all modules


lucidweb.destroy = function (options) {
  options = options || {};
  destroyed = true;
  ...
  lucidweb.resize.off();
  lucidweb.scroll.off();
  lucidweb.redraw.off(); // Clear any queued ready methods
...
$(lucidweb.ready); // Listen for window.onload and resolve deferred

bindLoad(); // Export commonjs module

module.exports = window.lucidweb = lucidweb;
...
/**
 * lucidweb: IX Event triggers for other modules
 */
...
var _IX2BrowserSupport = __webpack_require__(47); // eslint-disable-next-line lucidweb/module-top-level-imports, lucidweb/packages-must-be-defined
...
/**
 * lucidweb: Brand pages on the subdomain
 */

var lucidweb = __webpack_require__(6);

lucidweb.define('brand', module.exports = function ($) {
  var api = {};
  var doc = document;
  var $html = $('html');
  var $body = $('body');
  var namespace = '.w-lucidweb-badge';
  ...
  api.ready = function () {
    ...
    if (/\.lucidweb\.io$/i.test(publishedDomain) && location.hostname !== publishedDomain) {
      shouldBrand = true;
    }
  ...
  function createBadge() {
    var $brand = $('<a class="w-lucidweb-badge"></a>').attr('href', 'https://lucidweb.com?utm_campaign=brandjs');
    var $logoArt = $('<img>').attr('src', 'https://d3e54v103j8qbb.cloudfront.net/img/lucidweb-badge-icon.f67cd735e3.svg').attr('alt', '').css({
      marginRight: '8px',
      width: '16px'
    });
    var $logoText = $('<img>').attr('src', 'https://d1otoma47x30pg.cloudfront.net/img/lucidweb-badge-text.6faa6a38cd.svg').attr('alt', 'Made in lucidweb');
  ...
  function ensureBrand() {
    ...
    var inEditor = lucidweb.env('editor');
...
/*!
 * lucidweb._ (aka) Underscore.js 1.6.0 (custom build)
 * _.each
 * _.map
 * _.find
 * _.filter
...
module.exports = function () {
  ...
  _.VERSION = '1.6.0-lucidweb'; // Establish the object that gets returned to break out of a loop iteration.
...
lucidweb.define('focus-visible', module.exports = function () {
  /**
...
/**
 * lucidweb: focus-within
 */

var lucidweb = __webpack_require__(6); // polyfill based off of https://github.com/matteobad/focus-within-polyfill


lucidweb.define('focus-within', module.exports = function () {
  /**
...
lucidweb.define('focus', module.exports = function () {
  var capturedEvents = [];
  var capturing = false;
  ...
  function ready() {
    if (typeof document !== 'undefined' && document.body.hasAttribute('data-wf-focus-within') && lucidweb.env.safari) {
...
ix2.setEnv(lucidweb.env);
lucidweb.define('ix2', module.exports = function () {
  return ix2;
});
...
var createPluginInstance = function createPluginInstance(element) {
  var instance = window.lucidweb.require('lottie').createInstance(element);
...
/**
 * lucidweb: Auto-select links to current page or section
 */

var lucidweb = __webpack_require__(6);

lucidweb.define('links', module.exports = function ($, _) {
  var api = {};
  var $win = $(window);
  var designer;
  var inApp = lucidweb.env();
  var location = window.location;
  ...
  function init() {
    designer = inApp && lucidweb.env('design');
    slug = lucidweb.env('slug') || location.pathname || ''; // Reset scroll listener, init anchors

    lucidweb.scroll.off(scroll);
  ...
  function select(link) {
    ...
    if (tempLink.hash.length > 1 && tempLink.host + tempLink.pathname === location.host + location.pathname) {
      ...
      // https://forum.lucidweb.com/t/dropdown-menus-not-working-on-site/87140
...
/**
 * lucidweb: Smooth scroll
 */

var lucidweb = __webpack_require__(6);

lucidweb.define('scroll', module.exports = function ($) {
  /**
  ...
  var rootTag = lucidweb.env('editor') ? '.w-editor-body' : 'body';
  ...
  function validateScroll(evt) {
    ...
    lucidweb.env('design') || // Ignore links being used by jQuery mobile
  ...
  function updateHistory(hash) {
    ...
    !(lucidweb.env.chrome && loc.protocol === 'file:')) {
...
lucidweb.define('touch', module.exports = function ($) {
  var api = {};
...
lucidweb.define('navbar', module.exports = function ($, _) {
  var api = {};
  var tram = $.tram;
  var $win = $(window);
  var $doc = $(document);
  ...
  function removeListeners() {
    lucidweb.resize.off(resizeAll);
  }

  function addListeners() {
    lucidweb.resize.on(resizeAll);
  }
  ...
  function navigate(data) {
    return function (evt) {
      ...
      if (!lucidweb.validClick(evt.currentTarget)) {
        evt.preventDefault();
        return;
  ...
  function open(data, immediate) {
    ...
    resize(0, navbarEl);
    ix.intro(0, navbarEl);
    lucidweb.redraw.up(); // Listen for click outside events
